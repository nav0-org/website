...
tldr: >-
  Safari is the most privacy-friendly mainstream browser, but its privacy comes on Apple's terms. It still contacts Apple servers for Safe Browsing, Siri Suggestions, iCloud sync, and updates, and defaults to Google search - a deal worth an estimated $20 billion a year. The browser itself is closed source, tied to your Apple ID, and locked to Apple platforms. Nav0 is open source, collects zero telemetry, stores data locally, defaults to DuckDuckGo, and runs on macOS, Windows, and Linux.
